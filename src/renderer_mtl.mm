...
namespace bgfx { namespace mtl
{
	...
	struct RendererContextMtl : public RendererContextI
	{
		...
		bool init()
		{
			...
			createFrameBuffer(m_fbh, g_platformData.nwh, 0, 0, TextureFormat::D16);
...
//				support |= BGFX_CAPS_FORMAT_TEXTURE_IMAGE : BGFX_CAPS_FORMAT_TEXTURE_NONE;
		...
		void createFrameBuffer(FrameBufferHandle _handle, void* _nwh, uint32_t _width, uint32_t _height, TextureFormat::Enum _depthFormat) BX_OVERRIDE
		{
			...
			if (!isValid(_handle))
			{
				m_mainFrameBuffer.create(denseIdx, _nwh, _width, _height, _depthFormat);
			}
			else
			{
				m_frameBuffers[_handle.idx].create(denseIdx, _nwh, _width, _height, _depthFormat);
				m_frameBuffers[_handle.idx].m_swapChain->resize(m_frameBuffers[_handle.idx], _width, _height, 0);
			}
		}
...
//#define GROUP_VIEW_MARKERS
		...
		{
			m_mainFrameBuffer.m_swapChain->m_maxAnisotropy = !!(_resolution.m_flags & BGFX_RESET_MAXANISOTROPY)
				? 16
				: 1
				;
				...
				|| (m_resolution.m_flags&maskFlags) != (_resolution.m_flags&maskFlags) )
			{
				...
				m_mainFrameBuffer.m_swapChain->resize(m_mainFrameBuffer, _resolution.m_width, _resolution.m_height, _resolution.m_flags);
...
#undef CASE_IMPLEMENT_UNIFORM
		...
		SamplerState getSamplerState(uint32_t _flags)
		{
			...
			if (NULL == sampler)
			{
				...
				m_samplerDescriptor.maxAnisotropy =  (0 != (_flags & (BGFX_TEXTURE_MIN_ANISOTROPIC|BGFX_TEXTURE_MAG_ANISOTROPIC) ) ) ? m_mainFrameBuffer.m_swapChain->m_maxAnisotropy : 1;
	...
	RenderPipelineState ProgramMtl::getRenderPipelineState(uint64_t _state, uint32_t _rgba, FrameBufferHandle _fbHandle, VertexDeclHandle _declHandle,  uint16_t _numInstanceData)
	{
		...
		{
			murmur.add(s_renderMtl->m_mainFrameBuffer.m_pixelFormatHash);
		}
		else
		{
		...
		if (NULL == rps)
		{
			...
			if (!isValid(_fbHandle) || s_renderMtl->m_frameBuffers[_fbHandle.idx].m_swapChain)
			{
				SwapChainMtl* swapChain =
					!isValid(_fbHandle) ?
						s_renderMtl->m_mainFrameBuffer.m_swapChain :
						s_renderMtl->m_frameBuffers[_fbHandle.idx].m_swapChain;
...
#if BX_PLATFORM_IOS
			{
				if (![_nwh isKindOfClass:NSClassFromString(@"CAMetalLayer")])
				{
					BX_WARN(NULL != s_renderMtl->m_device, "Unable to create Metal device. Please set platform data window to a CAMetalLayer");
					return;
				}
				m_metalLayer = (CAMetalLayer *)_nwh;
			}
					...
					if ( NULL == m_renderCommandEncoder || fbh.idx != _render->m_fb[view].idx )
					{
						...
						if ( clearWithRenderPass )
						{
							...
							if (NULL != depthAttachment.texture)
							{
								...
								: MTLLoadActionLoad
								;
								depthAttachment.storeAction = NULL != m_mainFrameBuffer.m_swapChain->m_backBufferColorMSAA ? MTLStoreActionDontCare : MTLStoreActionStore;
							}
							...
							if (NULL != stencilAttachment.texture)
							{
								...
								: MTLLoadActionLoad
								;
								stencilAttachment.storeAction = NULL != m_mainFrameBuffer.m_swapChain->m_backBufferColorMSAA ? MTLStoreActionDontCare : MTLStoreActionStore;
							}
						}
						else
						{
